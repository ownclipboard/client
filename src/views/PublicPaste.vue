...
function addToThisDeviceClips(clipUuid: string) {
    if (!thisDeviceClips[pasteId.value])
...
async function loadThisDeviceClips() {
    ...
    try {
        const data = await $http.post<any, { clips: Pagination<OwnClip> }>(`/clips/find`, { ids });
        clips.value = data.clips;
        thisDeviceClips[pasteId.value] = data.clips.data.map((clip) => clip.uuid);
    } catch (e) {
        alertRequestError(e);
    }
}
        ...
        <section class="flex mt-5">
            ...
            >
                ...
                <template v-else>
                    ...
                    <h6
                        class="text-center text-gray-400 font-light mt-2 blink-slow"
                    >Click to paste TEXT or IMAGE</h6>
                </template>
            </div>
        </section>
